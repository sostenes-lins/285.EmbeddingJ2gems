...
In the next six figures we gather and display all data structure we need for the $O(n^2)$-algorithm
to PL-embed the dual of a $J^2$-gem.
\include{F.Appendix}
...
\begin{figure}[H] 
\begin{center}
\includegraphics[width=15 cm]{A.figs/coordX.pdf}
\caption{\sf Defining x coordinates.}
\label{fig:coordX}
\end{center}
\end{figure}
...
rectilinearly embedding one side of the last pair of wings 
(the other side is similar).
In the general step we have the black vertices which are in their fixed position.}
