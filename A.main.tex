...
\subsection{The Strategy for obtaining the PL-embedding we seek}
...
triangulated tetrahedron. After we use the inverse of a stereographic projection with center
in further in the exterior of the triangulated tetrahedron. In this way we recover in $\mathbb{S}^3$
...
seems very hard.
We split the algorithm into 3 phases. First we find a sequence of $n-1$ 2-dipole thickenings into
...
This sequence is easily obtainable from the
primal objects, the gems: $\mathcal{H}=\mathcal{H}_1, \mathcal{H}_2, \ldots, 
\mathcal{H}_{n-1}=\mathcal{B}_n.$ In the second phase we find specific abstract PL-triangulations
of the PL2-faces for the inverse sequence of colored 2-dimensional PL-complexes: 
$\mathcal{B}^\star_n=\mathcal{H}^\star_{n-1}, \ldots, 
\mathcal{H}^\star_2, \mathcal{H}^\star_{1}=\mathcal{H}^\star.$ At the same time that we get 
we get also another sequence $\mathcal{W}^\star_{n-1}, \ldots, 
\mathcal{W}^\star_2, \mathcal{W}^\star_{1}=\mathcal{W}^\star$. Each term of this
$\mathcal{W}$-sequence is called a {\em wing} and it is a section 
of the previous $\mathcal{H}$-sequence by two adequated semi-planes.
Each wing $\mathcal{W}_m$ has an associated {\em nervure}, denoted $\mathcal{N}_m$.
Each nervure is an auxilary spanning tree which helps in finding canonical embeddings for the
graph $\mathcal{U}_m = \mathcal{W}_m \cup \mathcal{N}_m$ ($m=n-1, \ldots, 1$), in 
two fixed semi-planes: $\Pi_\ell$ and $\Pi_r$.
The third phase uses the wings and the abstract 2-dimensional PL-complexes to produce 
a new sequence $\mathcal{H}^\diamond_{1},  
\mathcal{H}^\diamond_2, \ldots, \mathcal{H}^\diamond_{n}=\mathcal{H}^\star.$
...
\section{Abstract 2-dimensional PL-complexes, their wings and nervures}
\section{Diamond complexes}
