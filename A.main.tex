...
In the second phase we find specific abstract PL-triangulations
of the PL2-faces for the index increasing sequence of abstract colored 2-dimensional PL-complexes: 
$$\mathcal{B}^\star=\mathcal{H}^\star_{1} \mapright{bp}{move_1}  \mathcal{H}^\star_{2}  \mapright{bp}{move_{2}} \ldots
\mapright{bp}{move_{n-1}} \mathcal{H}^\star_{n}=\mathcal{H}^\star.$$ At the same time that we get 
we get also a sequence of {\em wings}
$$\mathcal{W}^\star_{1} \mapright{wbp}{move_{1}} \mathcal{W}^\star_{2}  \mapright{wbp}{move_{2}} \ldots
\mapright{wbp}{move_{n-1}} \mathcal{W}^\star_{n}=\mathcal{W}^\star.$$ Each wing
$\mathcal{W}_m$'s corresponds to a section 
of the previous sequence $\mathcal{H}_m$'s by two adequated fixed semi-planes $\Pi_\ell$ and $\Pi_r$.
Each wing $\mathcal{W}_m$ has an associated {\em nervure}, denoted $\mathcal{N}_m$.
Each nervure is an auxilary spanning tree which helps in finding canonical embeddings for the planar
graph $\mathcal{U}_m = \mathcal{W}_m \cup \mathcal{N}_m$ ($m=n-1, \ldots, 1$), in 
$\Pi_\ell$ and $\Pi_r$.
The third phase uses the final wing and the sequence of $\mathcal{H}^\star_{m}$'s to produce 
...
is the PL-embedding that we seek. The whole procedure can be implemented as a formal 
